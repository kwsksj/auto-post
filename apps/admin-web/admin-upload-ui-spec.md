...
- API側でも同名・同義（別名含む、かな正規化）の重複を拒否し、二重作成を防止する。

### 別名編集

- 管理UIのタグ設定パネルから、既存タグの `別名` を更新できる。
- 入力はカンマ区切りで受け付け、空欄保存で別名をクリアできる。
...
- タグ新規作成時に同名・同義（別名含む）の重複がサーバー側で防止される。
- 管理UIからタグ別名を編集して保存できる。
